...
def train(x_train, y_train, batch_size, x_test, y_test, epochs, max_features, embed_size, embedding_matrix, maxlen, n): 
...
def train_best_model(study, x_train, y_train, batch_size, x_test, y_test, epochs, max_features, embed_size, embedding_matrix, maxlen):
    ...
    with mlflow.start_run():
        ...
        model.fit(x_train, y_train, batch_size = batch_size , validation_***REMOVED*** = (x_test,y_test) , epochs = epochs, callbacks=[learning_rate_reduction])
...
def register_best_model(EXPT_NAME, MLFLOW_TRACKING_URI, model_name):
    ...
    if len(client.get_latest_versions(name=model_name, stages=["Production"])) != 0:
        ...
        if run_acc > prod_acc:
            ...
            mlflow.register_model( 
                        model_uri = f"runs:/{cur_run_id}/model",
                        name = model_name
                )
    ...
    elif len(client.get_latest_versions(name=model_name, stages=["Production"])) == 0 and len(client.get_latest_versions(name=model_name, stages=["Staging"])) != 0:
        stag_ver = client.get_latest_versions(name=model_name, stages=["Staging"])[0]
        stag_run_id=stag_ver.run_id
        stag_acc = client.get_metric_history(stag_run_id, 'val_accuracy')[0].value
        run_acc =  client.get_metric_history(cur_run_id, 'val_accuracy')[0].value
        if run_acc > stag_acc:
            ...
            print('Promoting previous model to Production and Registering new model to Staging ...')
            mlflow.register_model( 
                    model_uri = f"runs:/{cur_run_id}/model",
                    name = model_name
            )
...
def main():
    ...
    no_evals, epochs, model_name, testing = args.n_evals, args.epochs, args.model_name, args.test
    ...
    ***REMOVED***frame = read_***REMOVED***(path)
    ...
    study = train(x_train, y_train, batch_size, x_test, y_test, epochs, max_features, embed_size, embedding_matrix, maxlen, no_evals)
    # model = train_best_model(study, x_train, y_train, batch_size, x_test, y_test, epochs, max_features, embed_size, embedding_matrix, maxlen)
